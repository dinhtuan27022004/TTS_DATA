...
from custom_tts.model import CFM, SemanticCFM, TextToSemantic
from custom_tts.model.utils import (
    get_tokenizer,
...
def load_checkpoint(model, ckpt_path, device: str, dtype=None, use_ema=True):
    ...
    if use_ema:
        ...
        checkpoint["model_state_dict"] = _adapt_state_dict_for_model(model, checkpoint["model_state_dict"])
...
def load_model(
    ...
    if vocab_file == "":
        vocab_file = str(files("custom_tts").joinpath("infer/examples/vocab.txt"))
    tokenizer = "custom"
...
def load_semantic_model(
    ...
    if vocab_file == "":
        base_vocab = "/home/reg/TTS_DATA/models/f5-tts-v0/vocab.txt"
        vocab_file = base_vocab if os.path.isfile(base_vocab) else str(files("custom_tts").joinpath("infer/examples/vocab.txt"))
    tokenizer = "custom"
    ...
    semantic_student = TextToSemantic(
        ...
        semantic_dim=semantic_cfg.get("dim", model_cfg.get("semantic_dim", 768)),
        hidden_dim=semantic_cfg.get("hidden_dim", 512),
        depth=semantic_cfg.get("depth", 4),
        heads=semantic_cfg.get("heads", 8),
        ff_mult=semantic_cfg.get("ff_mult", 4),
        dropout=semantic_cfg.get("dropout", 0.1),
    )
    model = SemanticCFM(
        cfm=cfm,
        semantic_student=semantic_student,
        vocab_char_map=vocab_char_map,
        semantic_loss_weight=semantic_cfg.get("loss_weight", 0.1),
    ).to(device)
