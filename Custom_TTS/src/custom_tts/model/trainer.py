...
from custom_tts.model import CFM
from custom_tts.model.dataset import DynamicBatchSampler, collate_fn
from custom_tts.model.utils import default, exists
...
class Trainer:
    ...
    def _adapt_state_dict_for_model(self, state_dict):
    ...
    def load_checkpoint(self):
        ...
        elif "pretrained_semantic_init.pt" in os.listdir(self.checkpoint_path):
            latest_checkpoint = "pretrained_semantic_init.pt"
        else:
        ...
        if self.is_main:
            try:
                ...
                # Allows bootstrapping SemanticCFM from a plain F5 checkpoint.
                pass
    ...
    def train(self, train_dataset: Dataset, num_workers=16, resumable_with_seed: int = None):
        if self.log_samples:
            from custom_tts.infer.utils_infer import cfg_strength, load_vocoder, nfe_step, sway_sampling_coef

            vocoder = load_vocoder(
                vocoder_name=self.vocoder_name, is_local=self.is_local_vocoder, local_path=self.local_vocoder_path
            )
            target_sample_rate = self._unwrap_model().mel_spec.target_sample_rate
            ...
            for batch in current_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    if "semantic" in batch:
                        model_kwargs["semantic_gt"] = batch["semantic"]
                ...
                if self.accelerator.is_local_main_process:
                    ...
                    if hasattr(unwrapped_model, "last_losses"):
                        metrics.update(
                            {
                                f"loss/{name}": value
                                for name, value in unwrapped_model.last_losses.items()
                            }
                        )
                ...
                if global_update % self.save_per_updates == 0 and self.accelerator.sync_gradients:
                    ...
                    if self.log_samples and self.accelerator.is_local_main_process:
                        ...
                        with torch.inference_mode():
                            generated, _ = self._unwrap_model().sample(
